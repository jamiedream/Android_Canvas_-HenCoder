...
public class Practice14MaskFilterView extends View {
    ...
    private MaskFilter blurNormal = new BlurMaskFilter(50, BlurMaskFilter.Blur.NORMAL);
    private MaskFilter blurInner = new BlurMaskFilter(50, BlurMaskFilter.Blur.INNER);
    private MaskFilter blurOuter = new BlurMaskFilter(50, BlurMaskFilter.Blur.OUTER);
    private MaskFilter blurSolid = new BlurMaskFilter(50, BlurMaskFilter.Blur.SOLID);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 第一个：NORMAL
        paint.setMaskFilter(blurNormal);
        canvas.drawBitmap(bitmap, 100, 50, paint);

        // 第二个：INNER
        paint.setMaskFilter(blurInner);
        canvas.drawBitmap(bitmap, bitmap.getWidth() + 200, 50, paint);

        // 第三个：OUTER
        paint.setMaskFilter(blurOuter);
        canvas.drawBitmap(bitmap, 100, bitmap.getHeight() + 100, paint);

        // 第四个：SOLID
        paint.setMaskFilter(blurSolid);
